...
import desafio.itau.exceptions.TransacaoInvalidaException;
...
public class TransacaoService {
    ...
    public void salvarTransacao(Transacao add) {
        ...
        if (add == null) {
            throw new TransacaoInvalidaException("Valor da transação não pode ser vazio");
        }
            ...
            throw new TransacaoInvalidaException("Data e hora da transação não pode ser nula");
        }
        else if (add.getDataHora().isAfter(OffsetDateTime.now())) {
            throw new TransacaoInvalidaException("Transação futura não permitida");
        }
    ...
    public void removerTransacao() {
        transacaoRepository.limparTransacao();
    }
    ...
    public EstatisticasReponse obterEstatisticas() {
        ...
        List<Transacao> transacoes = getTransacoes().stream()
